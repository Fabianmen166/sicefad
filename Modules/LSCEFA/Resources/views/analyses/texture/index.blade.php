...
@section('title', 'Gestión de Análisis de Textura')
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Gestión de Análisis de Textura</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('lscefa.technical.analyses.index') }}">Inicio</a></li>
                        <li class="breadcrumb-item active">Análisis de Textura</li>
                    </ol>
                </div>
            </div>
        </div>
                                ...
                                @forelse($processes as $process)
                                    ...
                                    @if($textureService && $textureService->status === 'pending')
                                        <tr data-process-id="{{ $process->process_id }}" data-service-type="texture">
                                            ...
                                            <td>
                                                <a href="{{ route('lscefa.technical.analyses.texture.process', ['processId' => $process->process_id, 'serviceId' => $textureService->service_id]) }}"
...
<script>
    $(document).ready(function() {
        ...
        $('#processSelectedBtn').on('click', function() {
            ...
            var batchUrl = "{{ route('lscefa.technical.analyses.texture.batch') }}?processes=" + selectedProcesses.join(',');
