...
@section('title', 'Revisión de Análisis de Carbono Orgánico')
...
<div class="content-wrapper p-0 m-0" style="max-width: 100%;">
    ...
    @if(isset($analisis_completos) && count($analisis_completos) > 1)
        <div class="alert alert-info">
            ...
            Este servicio contiene <strong>{{ $total_analisis }}</strong> análisis de carbono orgánico individuales.
        </div>
    @endif
                    ...
                    <div class="row g-3">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" class="form-control form-control-sm" id="proceso"
                                value="{{ $analysis->process->process_id ?? 'N/A' }}" readonly>
                        </div>
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" class="form-control form-control-sm" id="servicio"
                                value="{{ $detail->service->descripcion ?? 'Carbono Orgánico' }}" readonly>
                        </div>
                        <!-- Total de Análisis -->
                        <div class="form-group col-md-3">
            ...
            <div class="card mt-3 border-0 shadow-none">
                ...
                <div class="card-body p-3">
                    ...
                    <div class="border p-3 mb-3 rounded">
                        ...
                        <!-- Blanco del método -->
                        @if(isset($control['identificacion_bm']) || isset($control['valor_leido']) || 
                            isset($control['limite_cuantificacion_metodo']) || isset($control['aceptable_blanco']))
                        <div class="mb-3">
                            ...
                            <div class="row g-3">
                                ...
                                @endif
                                @if(isset($control['valor_leido']))
                                <div class="col-md-3">
                                    <label class="small">Valor leído</label>
                                    <input type="text" class="form-control form-control-sm" 
                                        value="{{ is_numeric($control['valor_leido']) ? number_format($control['valor_leido'], 2) : $control['valor_leido'] }}" readonly>
                                </div>
                                @endif
                        ...
                        <!-- Muestra fortificada -->
                        @if(isset($control['identificacion_mf']) || isset($control['valor_obtenido']) || 
                            isset($control['recuperacion']) || isset($control['aceptable_fortificada']))
                        <div class="mb-3">
                        ...
                        @if(isset($control['identificacion_mr']) || isset($control['valor_referencia']) || 
                            isset($control['valor_obtenido']) || isset($control['aceptable_referencia']))
                        <div class="mb-3">
                            <h6 class="text-primary">Material de Referencia</h6>
                            <div class="row g-3">
                        ...
                        <!-- Duplicado Muestra -->
                        @if(isset($control['identificacion_dm']) || isset($control['replica_1']) || 
                            isset($control['replica_2']) || isset($control['dpr']) || isset($control['aceptable_duplicado']))
                        <div class="mb-3">
                            ...
                            <div class="row g-3">
                                ...
                                @endif
                                @if(isset($control['replica_1']))
                                <div class="col-md-3">
                                    <label class="small">Réplica 1</label>
                                    <input type="text" class="form-control form-control-sm" 
                                        value="{{ is_numeric($control['replica_1']) ? number_format($control['replica_1'], 2) : $control['replica_1'] }}" readonly>
                                </div>
                                @endif
                                @if(isset($control['replica_2']))
                                <div class="col-md-3">
                                    <label class="small">Réplica 2</label>
                                    <input type="text" class="form-control form-control-sm" 
                                        value="{{ is_numeric($control['replica_2']) ? number_format($control['replica_2'], 2) : $control['replica_2'] }}" readonly>
                                </div>
                                @endif
                        ...
                        <table class="table table-bordered text-center" id="tablaMuestras">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th>Volumen Sulfato Blanco (mL)</th>
                                    <th>Volumen Sulfato Muestra (mL)</th>
                                    <th>Volumen Dicromato (mL)</th>
                                    <th>Molaridad Sulfato</th>
                                    <th>% CO Total</th>
                                    <th>% COT</th>
                                    <th>% MO</th>
                                    <th>% Humedad</th>
                            ...
                            <tbody id="muestras-body">
                                ...
                                <tr class="muestra-fila">
                                    ...
                                    <td>
                                        {{ is_numeric($analisis['volumen_sulfato_blanco'] ?? $analisis->volumen_sulfato_blanco) ? number_format($analisis['volumen_sulfato_blanco'] ?? $analisis->volumen_sulfato_blanco, 2) : ($analisis['volumen_sulfato_blanco'] ?? $analisis->volumen_sulfato_blanco ?? 'N/A') }}
                                    </td>
                                    <td>
                                        {{ is_numeric($analisis['volumen_sulfato_muestra'] ?? $analisis->volumen_sulfato_muestra) ? number_format($analisis['volumen_sulfato_muestra'] ?? $analisis->volumen_sulfato_muestra, 2) : ($analisis['volumen_sulfato_muestra'] ?? $analisis->volumen_sulfato_muestra ?? 'N/A') }}
                                    </td>
                                    <td>
                                        {{ is_numeric($analisis['volumen_dicromato'] ?? $analisis->volumen_dicromato) ? number_format($analisis['volumen_dicromato'] ?? $analisis->volumen_dicromato, 2) : ($analisis['volumen_dicromato'] ?? $analisis->volumen_dicromato ?? 'N/A') }}
                                    </td>
                                    <td>
                                        {{ is_numeric($analisis['molaridad_sulfato'] ?? $analisis->molaridad_sulfato) ? number_format($analisis['molaridad_sulfato'] ?? $analisis->molaridad_sulfato, 4) : ($analisis['molaridad_sulfato'] ?? $analisis->molaridad_sulfato ?? 'N/A') }}
                                    </td>
                                    <td>
                                        <span class="badge bg-primary">
                                            {{ is_numeric($analisis['porcentaje_co_total'] ?? $analisis->porcentaje_co_total) ? number_format($analisis['porcentaje_co_total'] ?? $analisis->porcentaje_co_total, 2) : ($analisis['porcentaje_co_total'] ?? $analisis->porcentaje_co_total ?? 'N/A') }}%
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge bg-success">
                                            {{ is_numeric($analisis['porcentaje_cot'] ?? $analisis->porcentaje_cot) ? number_format($analisis['porcentaje_cot'] ?? $analisis->porcentaje_cot, 2) : ($analisis['porcentaje_cot'] ?? $analisis->porcentaje_cot ?? 'N/A') }}%
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge bg-info">
                                            {{ is_numeric($analisis['porcentaje_mo'] ?? $analisis->porcentaje_mo) ? number_format($analisis['porcentaje_mo'] ?? $analisis->porcentaje_mo, 2) : ($analisis['porcentaje_mo'] ?? $analisis->porcentaje_mo ?? 'N/A') }}%
                                        </span>
                                    </td>
                                    <td>
                                ...
                                @empty
                                <tr>
                                    <td colspan="13" class="text-center">No hay análisis de carbono registrados</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('lscefa.quality.reviews.accept', $analysis->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>¿Está seguro de aprobar este análisis de carbono orgánico?</p>
                        <div class="form-group">
                            ...
                            <input type="hidden" name="analysis_type" value="carbon">
                        </div>
                    </div>
